...
class BookingRepository extends BaseRepository
{
    ...
    public function store($cuser, $data)
    {
        ...
        $response['id'] = $job->id;
        $data = $this->getJobForData($data, $job);
    ...
    public function storeJobEmail($data)
    {
        ...
        $job = Job::findOrFail(Arr::get($data, 'user_email_job_id'));
        $job->user_email = Arr::get($data, 'user_email');
        $job->reference = Arr::get($data, 'reference', '');
        $user = $job->user()->get()->first();
        if (isset($data['address'])) {
            $job->address = Arr::get($data, 'address', $user->userMeta->address);
            $job->instructions = Arr::get($data, 'instructions', $user->userMeta->instructions);
            $job->town = Arr::get($data, 'town', $user->userMeta->city);
        }
        $job->save();
    ...
    {
        $data = $job->toArray();            // save job's information to data for sending Push
        $data['job_id'] = $job->id;
        $data['customer_town'] = $job->town;
        $data['customer_type'] = $job->user->userMeta->customer_type;

        $due_date = Carbon\Carbon::parse($job->due);
        $data['due_date'] = $due_date->format('Y-m-d');
        $data['due_time'] = $due_date->format('H:i:s');
    ...
    public function getPotentialJobIdsWithUserId($user_id)
    {
        ...
        $langIds = collect($languages)->pluck('lang_id')->all();
        // todo: refactor Job.php so that it returns Jobs instead of job ids. Very bad for performance
        $job_ids = Job::getJobs($user_id, $job_type, 'pending', $langIds, $user_meta->gender, $user_meta->translator_level);
        foreach ($job_ids as $k => $v) { // checking translator town
            $job = Job::find($v->id); // todo: unnecessary db queries
            $checktown = Job::checkTowns($job->user_id, $user_id);
            if (in_array($job->customer_phone_type, ['no', '']) && $job->customer_physical_type == 'yes' && !$checktown) {
                unset($job_ids[$k]);
            }
        }
    ...
    public function sendNotificationTranslator($job, $data = [], $exclude_user_id)
    {
        ...
        foreach ($users as $oneUser) {
            ...
            foreach ($jobs as $oneJob) {
                if ($job->id !== $oneJob->id) continue; // one potential job is the same with current job
...
//        }
     ...
     * @return mixed
     */
    public function getJobForData($data, $job)
    {
        $data['job_for'] = array();
        if ($job->gender != null) {
            if ($job->gender == 'male') {
